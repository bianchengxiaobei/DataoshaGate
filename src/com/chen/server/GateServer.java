...
public class GateServer extends MinaServer
{
	...
	public GateServer()
	{
		this(defaultMinaServerConfig,defaultInnerServerConfig,defaultGameConfig);
	}
	public GateServer(String minaServerConfig,String innerServerConfig,String gameConfig)
	{
	...
	public void sessionClosed(IoSession session) {
		...
		{
			stringBuffer.append("玩家IP:"+session.getAttribute("session_ip"));
		}
		if (session.containsAttribute("player_id"))
		{
			stringBuffer.append("玩家ID:"+session.getAttribute("player_id"));
		}
		if (session.containsAttribute("user_id"))
		{
			stringBuffer.append("玩家角色ID:"+session.getAttribute("user_id"));
		}
		messagelog.debug("玩家Session:"+session+",时间:"+System.currentTimeMillis()+"关闭连接");
		boolean quit = false;
		...
		public void run() {
			try {
				...
				}else
				{
					...
					MessageUtil.send_to_game(player.getCreateServer(), sessionId, msg);
				}
			} catch (Exception e) {
				log.error(e,e);
			}
